...
describe("Timelock", function () {
  ...
  it("updateUsdgSupply", async () => {
    ...
    await usdg.mint(zlpManager.address, 1000)

    expect(await usdg.balanceOf(zlpManager.address)).eq(1000)
    ...
    expect(await usdg.balanceOf(zlpManager.address)).eq(500)
    expect(await usdg.totalSupply()).eq(500)

    await timelock.updateUsdgSupply(2000)

    expect(await usdg.balanceOf(zlpManager.address)).eq(2000)
  ...
  it("initZlpManager", async () => {
    await expect(timelock.connect(user0).initZlpManager())
      .to.be.revertedWith("Timelock: forbidden")

      await zlp.setGov(timelock.address)
      await usdg.setGov(timelock.address)

      expect(await zlp.isMinter(zlpManager.address)).eq(false)
      expect(await usdg.vaults(zlpManager.address)).eq(false)
      expect(await vault.isManager(zlpManager.address)).eq(false)

      await timelock.initZlpManager()

      expect(await zlp.isMinter(zlpManager.address)).eq(true)
      expect(await usdg.vaults(zlpManager.address)).eq(true)
      expect(await vault.isManager(zlpManager.address)).eq(true)
  })
  ...
  it("initRewardRouter", async () => {
    await expect(timelock.connect(user0).initRewardRouter())
      ...
      expect(await stakedZlpTracker.isHandler(rewardRouter.address)).eq(false)
      expect(await feeZlpTracker.isHandler(rewardRouter.address)).eq(false)
      expect(await zlpManager.isHandler(rewardRouter.address)).eq(false)

      await timelock.initRewardRouter()

      expect(await stakedZlpTracker.isHandler(rewardRouter.address)).eq(true)
      expect(await feeZlpTracker.isHandler(rewardRouter.address)).eq(true)
      expect(await zlpManager.isHandler(rewardRouter.address)).eq(true)
  })
  ...
  it("setInPrivateTransferMode", async () => {
    ...
    await expect(timelock.connect(user0).setInPrivateTransferMode(zke.address, true))
      .to.be.revertedWith("Timelock: forbidden")

    await expect(timelock.connect(wallet).setInPrivateTransferMode(zke.address, true))
      .to.be.revertedWith("BaseToken: forbidden")

    await zke.setGov(timelock.address)

    expect(await zke.inPrivateTransferMode()).eq(false)
    await timelock.connect(wallet).setInPrivateTransferMode(zke.address, true)
    expect(await zke.inPrivateTransferMode()).eq(true)

    await timelock.connect(wallet).setInPrivateTransferMode(zke.address, false)
    expect(await zke.inPrivateTransferMode()).eq(false)

    await timelock.connect(wallet).setInPrivateTransferMode(zke.address, true)
    expect(await zke.inPrivateTransferMode()).eq(true)

    await expect(zke.connect(user0).transfer(user1.address, 100))
      .to.be.revertedWith("BaseToken: msg.sender not whitelisted")

    await timelock.connect(wallet).setInPrivateTransferMode(zke.address, false)
    expect(await zke.inPrivateTransferMode()).eq(false)

    await zke.connect(user0).transfer(user1.address, 100)
  })
  ...
  it("setShortsTrackerAveragePriceWeight", async () => {
    await zlpManager.setGov(timelock.address)
    expect(await zlpManager.gov()).eq(timelock.address)

    await expect(timelock.connect(user0).setShortsTrackerAveragePriceWeight(1234))
      .to.be.revertedWith("Timelock: forbidden")

    expect(await zlpManager.shortsTrackerAveragePriceWeight()).eq(0)
    await timelock.setShortsTrackerAveragePriceWeight(1234)
    expect(await zlpManager.shortsTrackerAveragePriceWeight()).eq(1234)
  })

  it("setZlpCooldownDuration", async () => {
    await zlpManager.setGov(timelock.address)
    expect(await zlpManager.gov()).eq(timelock.address)

    await expect(timelock.connect(user0).setZlpCooldownDuration(3600))
      .to.be.revertedWith("Timelock: forbidden")

    await expect(timelock.connect(wallet).setZlpCooldownDuration(3 * 60 * 60))
      .to.be.revertedWith("Timelock: invalid _cooldownDuration")

    expect(await zlpManager.cooldownDuration()).eq(86400)
    await timelock.setZlpCooldownDuration(3600)
    expect(await zlpManager.cooldownDuration()).eq(3600)
  })
})
