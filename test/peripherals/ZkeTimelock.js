...
describe("ZkeTimelock", function () {
  ...
  beforeEach(async () => {
    ...
    timelock = await deployContract("ZkeTimelock", [
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
  ...
  it("inits", async () => {
    ...
    await expect(deployContract("ZkeTimelock", [
      wallet.address,
      7 * 24 * 60 * 60 + 1,
      7 * 24 + 60 * 60,
      rewardManager.address,
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("ZkeTimelock: invalid _buffer")
    ...
    await expect(deployContract("ZkeTimelock", [
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("ZkeTimelock: invalid _longBuffer")
  })
  ...
  it("setTokenConfig", async () => {
    ...
    await expect(timelock.connect(wallet).setTokenConfig(
      ...
      0,
      0
    )).to.be.revertedWith("ZkeTimelock: token not yet whitelisted")
  ...
  it("setBuffer", async () => {
    const timelock0 = await deployContract("ZkeTimelock", [
      user1.address,
      3 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      rewardManager.address,
    ...
    await expect(timelock0.connect(user1).setBuffer(3 * 24 * 60 * 60 - 10))
      .to.be.revertedWith("ZkeTimelock: buffer cannot be decreased")
  ...
  it("setMaxLeverage", async () => {
    ...
    await expect(timelock.connect(wallet).setMaxLeverage(vault.address, 49 * 10000))
      .to.be.revertedWith("ZkeTimelock: invalid _maxLeverage")
  ...
  it("addExcludedToken", async () => {
    const zke = await deployContract("ZKE", [])
    await expect(timelock.connect(user0).addExcludedToken(zke.address))
      .to.be.revertedWith("ZkeTimelock: forbidden")

    expect(await timelock.excludedTokens(zke.address)).eq(false)
    await timelock.connect(wallet).addExcludedToken(zke.address)
    expect(await timelock.excludedTokens(zke.address)).eq(true)
  })
  ...
  it("setInPrivateTransferMode", async () => {
    ...
    await timelock.addExcludedToken(zke.address)
    await expect(timelock.connect(wallet).setInPrivateTransferMode(zke.address, true))
      .to.be.revertedWith("ZkeTimelock: invalid _inPrivateTransferMode")
  ...
  it("setExternalAdmin", async () => {
    ...
    await expect(timelock.connect(wallet).setExternalAdmin(timelock.address, user3.address))
      .to.be.revertedWith("ZkeTimelock: invalid _target")
  })
