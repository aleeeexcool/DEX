...
vault = await deployVault()
  ...
  async function debugState(label = "") {
    ...
    const aum = await zlpManager.getAum(true)
  ...
  it("PositionManager and ZlpManager init with shortsTracker", async () => {
    const [
      positionManagerShortTracker,
      zlpManagerShortTracker,
      avgeragePrice,
      size
    ] = await Promise.all([
      positionManager.shortsTracker(),
      zlpManager.shortsTracker(),
    ...
    expect(zlpManagerShortTracker, 'zlpManager shortsTracker').eq(shortsTracker.address)
