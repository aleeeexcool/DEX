...
describe("Vault.increaseLongPosition", function () {
  ...
  it("increasePosition long", async () => {
    ...
    expect(await zlpManager.getAumInUsdg(true)).eq("48029860000000000000") // 48.02986
  ...
  it("increasePosition long aum", async () => {
    ...
    expect(await zlpManager.getAumInUsdg(false)).eq(expandDecimals(64890, 18)) // 30080 + (1.4962-0.8)*50000
    ...
    expect(await zlpManager.getAumInUsdg(true)).eq("102295000500000000000000") // 102295.0005
    expect(await zlpManager.getAumInUsdg(false)).eq("68196667000000000000000") // 68196.667
