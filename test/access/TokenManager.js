...
describe("TokenManager", function () {
  ...
  beforeEach(async () => {
    ...
    zkeTimelock = await deployContract("ZkeTimelock", [
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      user0.address,
  ...
  it("approve", async () => {
    ...
    await expect(tokenManager.connect(wallet).approve(zke.address, user2.address, expandDecimals(5, 18), 1))
      .to.be.revertedWith("TokenManager: action not signalled")
  ...
  it("signalSetGov", async () => {
    await expect(tokenManager.connect(user0).signalSetGov(timelock.address, zke.address, user1.address))
      .to.be.revertedWith("TokenManager: forbidden")

    await tokenManager.connect(wallet).signalSetGov(timelock.address, zke.address, user1.address)
  })

  it("signSetGov", async () => {
    await expect(tokenManager.connect(user0).signSetGov(timelock.address, zke.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: forbidden")

    await expect(tokenManager.connect(signer2).signSetGov(timelock.address, zke.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")
    ...
    await tokenManager.connect(signer2).signSetGov(timelock.address, zke.address, user1.address, 1)

    await expect(tokenManager.connect(signer2).signSetGov(timelock.address, zke.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: already signed")

    await tokenManager.connect(signer1).signSetGov(timelock.address, zke.address, user1.address, 1)
  })

  it("setGov", async () => {
    await zke.setGov(zkeTimelock.address)

    await expect(tokenManager.connect(user0).setGov(zkeTimelock.address, zke.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: forbidden")

    await expect(tokenManager.connect(wallet).setGov(zkeTimelock.address, zke.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await tokenManager.connect(wallet).signalSetGov(zkeTimelock.address, zke.address, user1.address)

    await expect(tokenManager.connect(wallet).setGov(user2.address, zke.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(zkeTimelock.address, user0.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(zkeTimelock.address, zke.address, user2.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(zkeTimelock.address, zke.address, user1.address, 1 + 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(zkeTimelock.address, zke.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not authorized")

    await tokenManager.connect(signer0).signSetGov(zkeTimelock.address, zke.address, user1.address, 1)

    await expect(tokenManager.connect(wallet).setGov(zkeTimelock.address, zke.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: insufficient authorization")

    await expect(zkeTimelock.connect(wallet).signalSetGov(zke.address, user1.address))
      .to.be.revertedWith("ZkeTimelock: forbidden")

    await tokenManager.connect(signer2).signSetGov(zkeTimelock.address, zke.address, user1.address, 1)

    await expect(zkeTimelock.connect(wallet).setGov(zke.address, user1.address))
      .to.be.revertedWith("ZkeTimelock: action not signalled")

    await tokenManager.connect(wallet).setGov(zkeTimelock.address, zke.address, user1.address, 1)
    ...
    expect(await zke.gov()).eq(zkeTimelock.address)
    await zkeTimelock.connect(wallet).setGov(zke.address, user1.address)
    expect(await zke.gov()).eq(user1.address)
  })
})
