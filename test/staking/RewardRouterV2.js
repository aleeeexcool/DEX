...
describe("RewardRouterV2", function () {
  ...
  beforeEach(async () => {
    ...
    zkeVester = await deployContract("Vester", [
      "Vested ZKE", // _name
      "vZKE", // _symbol
    ...
    zlpVester = await deployContract("Vester", [
      "Vested ZLP", // _name
      "vZLP", // _symbol
      vestingDuration, // _vestingDuration
      esZke.address, // _esToken
  ...
  it("handleRewards", async () => {
    ...
    await esZke.setHandler(rewardRouterV2.address, true)
    await esZke.setHandler(stakedZkeDistributor.address, true)
    await esZke.setHandler(stakedZlpDistributor.address, true)
    await esZke.setHandler(stakedZkeTracker.address, true)
    await esZke.setHandler(stakedZlpTracker.address, true)
    ...
    await zlpManager.setHandler(rewardRouterV2.address, true)
    await stakedZkeTracker.setHandler(rewardRouterV2.address, true)
    await bonusZkeTracker.setHandler(rewardRouterV2.address, true)
    await feeZkeTracker.setHandler(rewardRouterV2.address, true)
    await feeZlpTracker.setHandler(rewardRouterV2.address, true)
    await stakedZlpTracker.setHandler(rewardRouterV2.address, true)

    await esZke.setHandler(rewardRouterV2.address, true)
    await bnZke.setMinter(rewardRouterV2.address, true)
    await esZke.setMinter(zkeVester.address, true)
    await esZke.setMinter(zlpVester.address, true)

    await zkeVester.setHandler(rewardRouterV2.address, true)
    await zlpVester.setHandler(rewardRouterV2.address, true)
    ...
    await bnb.mint(user1.address, expandDecimals(1, 18))
    await bnb.connect(user1).approve(zlpManager.address, expandDecimals(1, 18))
    await rewardRouterV2.connect(user1).mintAndStakeZlp(
      bnb.address,
      expandDecimals(1, 18),
      expandDecimals(299, 18),
      expandDecimals(299, 18)
    ...
    expect(await zke.balanceOf(user1.address)).eq(expandDecimals(200, 18))
    await zke.connect(user1).approve(stakedZkeTracker.address, expandDecimals(200, 18))
    await rewardRouterV2.connect(user1).stakeZke(expandDecimals(200, 18))
    expect(await zke.balanceOf(user1.address)).eq(0)
  ...
  it("StakedZlp", async () => {
    ...
    const stakedZlp = await deployContract("StakedZlp", [zlp.address, zlpManager.address, stakedZlpTracker.address, feeZlpTracker.address])
  ...
  it("FeeZlp", async () => {
    ...
    const zlpBalance = await deployContract("ZlpBalance", [zlpManager.address, stakedZlpTracker.address])
