...
describe("WeeklyVesting", () => {
    ...
    it("should allow user to buy tokens", async () => {
        ...
        // expect(BigNumber.from(res[0]).eq(parseEther("10"))).to.be.true;
        // expect(BigNumber.from(res[1]).eq(parseEther("0"))).to.be.true;
        // expect(BigNumber.from(res[2]).eq(parseEther("0"))).to.be.true;
        // // expect(await weeklyVesting.participants(user1.address)).to.include({
        // //     zkePurchased: BigNumber.equal(parseEther("10")),
        // //     zkeClaimed: parseEther("0"),
        // // });
    });
