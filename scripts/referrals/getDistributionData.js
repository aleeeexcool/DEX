...
const ARBITRUM_SUBGRAPH_ENDPOINT = 'https://api.thegraph.com/subgraphs/name/zke-io/zke-arbitrum-referrals'
const AVALANCHE_SUBGRAPH_ENDPOINT = 'https://api.thegraph.com/subgraphs/name/zke-io/zke-avalanche-referrals'

const BigNumber = ethers.BigNumber
const { formatUnits, parseUnits } = ethers.utils
const SHARE_DIVISOR = BigNumber.from("1000000000") // 1e9
const BONUS_TIER = 2 // for EsZKE distributions
...
async function queryDistributionData(network, fromTimestamp, toTimestamp, account, zkePrice, eszkeRewards) {
  if (zkePrice) {
    zkePrice = parseUnits(zkePrice, USD_DECIMALS)
  }
  if (eszkeRewards) {
    eszkeRewards = parseUnits(eszkeRewards, ZKE_DECIMALS)
  }
  ...
  if (zkePrice && eszkeRewards) {
    const eszkeRewardsUsdLimit = eszkeRewards.mul(zkePrice).div(expandDecimals(1, ZKE_DECIMALS))
    let eszkeRewardsUsdTotal = BigNumber.from(0)
    Object.values(referrersRebatesData).forEach(data => {
      if (data.tierId !== BONUS_TIER) {
        return
      }
      data.eszkeRewardsUsd = data.volume.div(1000).div(20) // 0.1% margin fee, 0.05% of fee is EsZKE bonus rewards
      data.eszkeRewards = data.eszkeRewardsUsd
        .mul(expandDecimals(1, USD_DECIMALS))
        .div(zkePrice)
        .div(expandDecimals(1, 12))
      eszkeRewardsUsdTotal = eszkeRewardsUsdTotal.add(data.eszkeRewardsUsd)
    })

    if (eszkeRewardsUsdTotal.gt(eszkeRewardsUsdLimit)) {
      const denominator = eszkeRewardsUsdTotal.mul(USD_DECIMALS).div(eszkeRewardsUsdLimit)
      Object.values(referrersRebatesData).forEach(data => {
        data.eszkeRewardsUsd = data.eszkeRewardsUsd.mul(USD_DECIMALS).div(denominator)
        data.eszkeRewards = data.eszkeRewardsUsd
          .mul(expandDecimals(1, USD_DECIMALS))
          .div(zkePrice)
          .div(expandDecimals(1, 12))
  ...
  for (const data of Object.values(referrersRebatesData)) {
    ...
    consoleData.push({
      ...
      tierId: data.tierId,
      "eszkeRewards, $": data.eszkeRewardsUsd ? formatUnits(data.eszkeRewardsUsd, USD_DECIMALS) : null,
      eszkeRewards: data.eszkeRewards ? formatUnits(data.eszkeRewards, ZKE_DECIMALS) : null,
    })
    output.referrers.push({
      ...
      tierId: data.tierId,
      eszkeRewards: data.eszkeRewards ? data.eszkeRewards.toString() : null,
      eszkeRewardsUsd: data.eszkeRewardsUsd ? data.eszkeRewardsUsd.toString() : null,
    })
  }
...
async function main() {
  ...
  parser.add_argument('-g', '--zke-price', { help: 'ZKE TWAP price' })
  parser.add_argument('-e', '--eszke-rewards', {
    help: 'Amount of EsZKE to distribute to Tier 3',
    default: "5000"
  })
