...
async function getArbValues() {
  ...
  const esZke = await contractAt("Token", "0xf42Ae1D54fd613C9bb14810b0588FaAa09a426cA")
  const nativeTokenPrice = ethPrice
  const data = arbitrumData

  return { batchSender, esZke, nativeTokenPrice, data }
}

async function getAvaxValues() {
  const batchSender = await contractAt("BatchSender", "0xF0f929162751DD723fBa5b86A9B3C88Dc1D4957b")
  const esZke = await contractAt("Token", "0xFf1489227BbAAC61a9209A08929E4c2a526DdD17")
...
async function main() {
  ...
  const { batchSender, esZke, nativeTokenPrice, data } = await getValues()
  ...
  let totalEsZkeAmount = bigNumberify(0)
  ...
  for (let i = 0; i < affiliatesData.length; i++) {
    const { account, rebateUsd, eszkeRewardsUsd } = affiliatesData[i]
    ...
    if (eszkeRewardsUsd) {
      const esZkeAmount = bigNumberify(eszkeRewardsUsd).mul(expandDecimals(1, 18)).div(expandDecimals(zkePrice, 30))
      esZkeAccounts.push(account)
      esZkeAmounts.push(esZkeAmount)
      totalEsZkeAmount = totalEsZkeAmount.add(esZkeAmount)
    }
  }
  ...
  console.log(`total esZke`, ethers.utils.formatUnits(totalEsZkeAmount, 18))
  ...
  if (shouldSendTxn) {
    ...
    await sendTxn(esZke.approve(batchSender.address, totalEsZkeAmount), "esZke.approve")

    await processBatch([esZkeAccounts, esZkeAmounts], batchSize, async (currentBatch) => {
      printBatch(currentBatch)

      const accounts = currentBatch.map((item) => item[0])
      const amounts = currentBatch.map((item) => item[1])

      await sendTxn(batchSender.sendAndEmit(esZke.address, accounts, amounts, affiliateRewardsTypeId), "batchSender.sendAndEmit(nativeToken, esZke affiliate rewards)")
    })
  }
}
