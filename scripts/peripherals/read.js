...
async function readZlpManager(zlpManager) {
  console.log("zlpManager.cooldownDuration", (await zlpManager.cooldownDuration()).toString())
}
...
async function main() {
  ...
  // await readZlpManager(zlpManager)

  // await getPool(weth.address, usdc.address, 500)
  // await getPool(weth.address, zke.address, 10000)
  ...
  startTime = Date.now()
  await zlpManager.getAums()
  console.log("zlpManager.getAums", Date.now() - startTime)
}
