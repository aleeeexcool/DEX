...
async function getZkePrice(ethPrice) {
...
async function getArbValues() {
  ...
  const stakedZke = await contractAt("Token", "0xd2D1162512F927a7e282Ef43a362659E4F2a728F", signer)
  const stakedZkeSupply = await stakedZke.totalSupply()
  const { totalTransferAmount: keeperCosts } = await getArbKeeperValues()
  const zlpManager = await contractAt("ZlpManager", "0x321F653eED006AD1C29D174e17d96351BDe22649", signer)
  const zlpAum = await zlpManager.getAum(true)

  return { vault, reader, tokens, tokenInfo, nativeTokenPrice, feesUsd, stakedZke, stakedZkeSupply, keeperCosts, zlpManager, zlpAum }
}
...
async function getAvaxValues() {
  ...
  const nativeTokenPrice = tokenInfo[tokens.nativeToken.address].maxPrice
  const feesUsd = await getFeesUsd(vault, reader, tokenInfo)
  const stakedZke = await contractAt("Token", "0x4d268a7d4C16ceB5a606c173Bd974984343fea13", signer)
  const stakedZkeSupply = await stakedZke.totalSupply()
  const { totalTransferAmount: keeperCosts } = await getAvaxKeeperValues()
  const zlpManager = await contractAt("ZlpManager", "0xe1ae4d4b06A5Fe1fc288f6B4CD72f9F8323B107F", signer)
  const zlpAum = await zlpManager.getAum(true)
...
async function main() {
  ...
  const zkePrice = await getZkePrice(ethPrice)

  const data = [
    ["ETH Price", formatAmount(ethPrice, 30, 2)],
    ["AVAX Price", formatAmount(avaxPrice, 30, 2)],
    ["ZKE Price", formatAmount(zkePrice, 30, 2)],
    ["ARB Fees", formatAmount(values.arbitrum.feesUsd, 30, 2)],
    ["AVAX Fees", formatAmount(values.avax.feesUsd, 30, 2)],
    ["ARB sbfZKE", formatAmount(values.arbitrum.stakedZkeSupply, 18, 2)],
    ["AVAX sbfZKE", formatAmount(values.avax.stakedZkeSupply, 18, 2)],
    ...
    ["ZLP AUM (ARB)", formatAmount(values.arbitrum.zlpAum, 30, 2)],
    ["ZLP AUM (AVAX)", formatAmount(values.avax.zlpAum, 30, 2)],
  ]
