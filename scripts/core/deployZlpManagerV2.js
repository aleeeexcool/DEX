...
async function getArbValues() {
  ...
  return { vault, usdg, zlp, shortsTracker }
}
...
async function main() {
  const { vault, usdg, zlp, shortsTracker } = await getValues()

  const cooldownDuration = 0
  const zlpManager = await deployContract("ZlpManager", [vault.address, usdg.address, zlp.address, shortsTracker.address, cooldownDuration])

  await sendTxn(zlpManager.setInPrivateMode(true), "zlpManager.setInPrivateMode")

  writeTmpAddresses({
    zlpManager: zlpManager.address
  })
}
