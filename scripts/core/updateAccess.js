...
async function main() {
  const stakedZkeTracker = await contractAt("RewardTracker", "0x2bD10f8E93B3669b6d42E74eEedC65dd1B0a1342")
  const bonusZkeTracker = await contractAt("RewardTracker", "0x908C4D94D34924765f1eDc22A1DD098397c59dD4")
  const feeZkeTracker = await contractAt("RewardTracker", "0x4d268a7d4C16ceB5a606c173Bd974984343fea13")
  ...
  await printRewardTracker(stakedZkeTracker, "stakedZkeTracker")
  await printRewardTracker(bonusZkeTracker, "bonusZkeTracker")
  await printRewardTracker(feeZkeTracker, "feeZkeTracker")

  await printRewardTracker(stakedZlpTracker, "stakedZlpTracker")
  await printRewardTracker(feeZlpTracker, "feeZlpTracker")

  const zlp = await contractAt("MintableBaseToken", "0x01234181085565ed162a948b6a5e88758CD7c7b8")
  const usdg = await contractAt("USDG", "0xc0253c3cC6aa5Ab407b5795a04c28fB063273894")
  // const zke = await contractAt("MintableBaseToken", "0x62edc0692BD897D2295872a9FFCac5425011c661")
  // const esZke = await contractAt("MintableBaseToken", "0xFf1489227BbAAC61a9209A08929E4c2a526DdD17")
  const bnZke = await contractAt("MintableBaseToken", "0x8087a341D32D445d9aC8aCc9c14F5781E04A26d2")

  await printToken(zlp, "zlp")
  await printUsdg(usdg, "usdg")
  // await printToken(zke, "zke")
  // await printToken(esZke, "esZke")
  await printToken(bnZke, "bnZke")
  ...
  // await signalGov(prevGov, zlp, nextGov, "zlp")
  // await signalGov(prevGov, zke, nextGov, "zke")
  // await signalGov(prevGov, esZke, nextGov, "esZke")
  // await signalGov(prevGov, bnZke, nextGov, "bnZke")

  await updateToken(zke, "zke")
  await updateToken(esZke, "esZke")
  await updateToken(bnZke, "bnZke")

  await updateHandler(stakedZkeTracker, "stakedZkeTracker")
  await updateHandler(bonusZkeTracker, "bonusZkeTracker")
  await updateHandler(feeZkeTracker, "feeZkeTracker")
  await updateHandler(stakedZlpTracker, "stakedZlpTracker")
  await updateHandler(feeZlpTracker, "feeZlpTracker")

  await updateRewardTrackerGov(stakedZkeTracker, "stakedZkeTracker")

  await updateRewardTrackerGov(bonusZkeTracker, "bonusZkeTracker")
  await updateRewardTrackerGov(feeZkeTracker, "feeZkeTracker")
  await updateRewardTrackerGov(stakedZlpTracker, "stakedZlpTracker")
  await updateRewardTrackerGov(feeZlpTracker, "feeZlpTracker")

  await updateGov(zlp, "zlp")
  await updateGov(usdg, "usdg")
  // await updateGov(zke, "zke")
  // await updateGov(esZke, "esZke")
  await updateGov(bnZke, "bnZke")
  ...
  await updateGov(zlpManager, "zlpManager")
