...
const stakeZkeList = require("../../data/stakeZkeList.json")
...
async function main() {
  ...
  for (let i = 0; i < stakeZkeList.length; i++) {
    const { address, balance } = stakeZkeList[i]

    const stakedAmount = await stakedZkeTracker.stakedAmounts(address)
    ...
    const pendingRewards = await stakedZkeTracker.claimable(address)
    const pendingBonus = await bonusZkeTracker.claimable(address)
