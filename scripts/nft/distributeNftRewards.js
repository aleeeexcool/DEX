...
const distribute = async () => {
  ...
  const stakedZkeBalances = await processFile(inputDir + "staked-zke-balances.csv")
  const vestedBalances = await processFile(inputDir + "vested-balances.csv")

  const holders = {}

  for (let i = 0; i < stakedZkeBalances.length; i++) {
    const tokenHolder = stakedZkeBalances[i]
  ...
  if (shouldSendTokens) {
    await sendTxn(esZke.approve(batchSender.address, expandDecimals(totalEsZke, 18)), "esZke.approve")
  }
  ...
  for (let i = 0; i < balanceList.length; i++) {
    ...
    const esZkeValue = (totalEsZke - 1) * balance / totalBalance
    const esZkeAmount = ethers.utils.parseUnits(esZkeValue.toFixed(4), 18)

    accounts.push(account)
    amounts.push(esZkeAmount)
    totalEsZkeAmount = totalEsZkeAmount.add(esZkeAmount)

    console.log(`${i+1},${account},${esZkeValue},${esZkeAmount.toString()}`)

    if (accounts.length === batchSize && shouldSendTokens) {
      console.log("sending batch", i, accounts.length, amounts.length)
      await sendTxn(batchSender.send(esZke.address,  accounts, amounts), "batchSender.send")
  ...
  console.log("totalEsZkeAmount", totalEsZkeAmount.toString())
}
