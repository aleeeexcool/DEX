...
contract EsWxtBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esWxt;

    constructor(address _esWxt) public {
        admin = msg.sender;
        esWxt = _esWxt;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsWxtBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esWxt).transferFrom(msg.sender, _accounts[i], _amounts[i]);
