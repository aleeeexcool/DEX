...
contract EsZkeBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esZke;

    constructor(address _esZke) public {
        admin = msg.sender;
        esZke = _esZke;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsZkeBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esZke).transferFrom(msg.sender, _accounts[i], _amounts[i]);
